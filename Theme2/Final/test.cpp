...
public:
    ...
    void PushFront(const Type& value) {
        ...
        bool first_node = IsEmpty();
       
        head_.next_node = new Node(value, head_.next_node);
        ++size_;
        first_node ? (last_ = head_.next_node) : 0;
    ...
    Iterator InsertAfter(ConstIterator pos, const Type& value) {
        ...
        auto it = new Node (value, pos.node_->next_node);

        if(pos.node_ == last_){
            last_ = it;
        }

        pos.node_->next_node = it;
        ++size_;
    ...
    void InitList(const _Container& container){
    ...
    //Удаляет элемент вначале списка
    ...
    Iterator EraseAfter(ConstIterator pos) noexcept {
        ...
        if(pos.node_ == last_ || pos == this->end()){
            return {};
        }
...
private:
    ...
    // Позволяет использовать функцию PushBack за O(1)
    Node* last_ = &head_; 
};
...
// Эти функции тестирует работу SingleLinkedList
void Test0() {
    using namespace std;
    {
